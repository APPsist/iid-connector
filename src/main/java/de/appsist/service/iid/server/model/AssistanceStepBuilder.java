...
public class AssistanceStepBuilder {
	...
	/**
	 * Adds a list of process titles to display.
	 * @param processTitles List of titles ordered top down.
	 * @return Builder instance.
	 */
	public AssistanceStepBuilder setProcessTitles(List<String> processTitles) {
		if (!processTitles.isEmpty()) {
			JsonArray processTitlesArray = new JsonArray();
			for (String entry : processTitles) {
				processTitlesArray.addString(entry);
			}
			json.putArray("processTitles", processTitlesArray);
		}
		return this;
	}
