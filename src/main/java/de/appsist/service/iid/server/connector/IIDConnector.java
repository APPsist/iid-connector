...
/**
 * Connector for the APPsist content interaction service.
 * @author simon.schwantzer(at)im-c.de
 */
public class IIDConnector {
	public static final String DEFAULT_ADDRESS = "appsist:service:iid";
     ...
     * 
     * @param sessionId
     *            Identifier of the user session showing the notification.
     * @param notificationId
     *            Identifier of the notification to dismiss.
     * @param resultHandler
     *            Handler to check if the operation succeeded. May be <code>null</code>.
     */
    public void purgeNotifications(String sessionId, AsyncResultHandler<Void> resultHandler)
    {
        JsonObject message = new JsonObject();
     ...
     * 
     * @param sessionId
     *            Identifier of the user session to display content in.
     * @param serviceId
     *            Identifier of the service which wants to display content.
     * @param assistance
     *            Assistance step to display.
     * @param resultHandler
     *            Handler to check if the operation succeeded. May be <code>null</code>.
     */
	...
	/**
	 * Returns the activity the user is currently evolved with.
	 * @param sessionId Identifier of the user session.
	 * @param resultHandler Handler for the activity information.
	 */
	public void getUserActivity(String sessionId, final AsyncResultHandler<Activity> resultHandler) {
		JsonObject message = new JsonObject()
			.putString("action", "getUserActivity")
			.putString("sessionId", sessionId);
		...
		eventBus.send(address, message, new Handler<Message<JsonObject>>() {
			...
			public void handle(Message<JsonObject> message) {
				final JsonObject body = message.body();
				resultHandler.handle(new AsyncResult<Activity>() {
					...
					public Activity result() {
						return succeeded() ? Activity.fromString(body.getString("activity")) : null;
					}
